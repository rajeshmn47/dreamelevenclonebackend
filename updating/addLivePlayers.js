...
module.exports.addLivePlayers = async function () {
    try {
        const matches = await Match.find({
            date: { $gte: new Date(Date.now() - 100 * 60 * 60 * 1000), $lt: new Date(Date.now() + 0.5 * 60 * 60 * 1000) },
        });
        ...
        for (let match of matches) {
            ...
            while (!success) {
                try {
                    ...
                    const team1Players = data.team1.players['playing XI'].map(p => ({
                        playerId: p.id,
                        playerName: p.name,
                        image: p.faceImageId,
                        points: 4,
                    ...
                    const team2Players = data.team2.players['playing XI'].map(p => ({
                        playerId: p.id,
                        playerName: p.name,
                        image: p.faceImageId,
                        points: 4,
